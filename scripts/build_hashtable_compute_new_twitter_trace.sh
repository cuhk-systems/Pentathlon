...
REGENERATE_LL="${REGENERATE_LL:-0}"

HASHTABLE_DIR="$REPO_ROOT/benchmarks-new/hashtable"
BUILD_DIR="$HASHTABLE_DIR/build"
INPUT_CPP="$HASHTABLE_DIR/main.cpp"
INPUT_LL="$BUILD_DIR/hashtable.input.ll"
OPT_LL="$HASHTABLE_DIR/hashtable.optimized.ll"
OBJ_OUT="$BUILD_DIR/hashtable.o"
LIB_OUT="$BUILD_DIR/libhashtable.a"
TWITTER_DIR="$REPO_ROOT/benchmarks-new/Twitter"
BENCHMARK_NAME="benchmark-hashtable-twitter-trace"
...
if [[ "$REGENERATE_LL" == "1" ]]; then
  echo "Compiling hashtable benchmark source to LLVM IR..."
  ...
  echo "Missing checked-in LLVM IR: $OPT_LL" >&2
  exit 1
else
  echo "Using checked-in LLVM IR: $OPT_LL"
fi
...
project(pentathlon_hashtable_compute_new_twitter_trace LANGUAGES C CXX)
